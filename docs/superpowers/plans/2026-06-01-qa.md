...
2. For each action/expect pair:
   ...
   - `agent-browser snapshot` to see what changed
   - `agent-browser console` to check for errors
   - Judge: does the observed result match the `<expect>` text?
...
2. **Execute operation** based on the element's ARIA role (see Action Strategy below).
3. **Wait**: `agent-browser wait 1000`
4. **Screenshot after**: `agent-browser screenshot {OUTPUT_DIR}/screenshots/step-{NNN}-after.png`
5. **Snapshot**: `agent-browser snapshot` to check what changed
6. **Console**: `agent-browser console` + `agent-browser errors`
7. **Judge** the interaction against the 7-Item Checklist and Issue Taxonomy (see references/issue-taxonomy.md)
